...
> = {
  ...
  resolve_task: {
    description:
      "Resolve a natural-language task reference to one exact task or a candidate list.",
    skills: ["task-update-delete"],
    keywords: ["resolve", "find task", "identify task", "match task"],
  },
  update_task: {
    description:
      "Update an existing task after an exact resolution in the current turn.",
    skills: ["task-update-delete"],
  ...
  get_energy_context: {
    description:
      "Return a compact energy summary with current level and next windows.",
    skills: [
      "energy-check",
      "task-prioritise",
      "task-create",
...
function aiTool<Schema extends z.ZodTypeAny, Output>(options: {
  ...
  return tool<z.infer<Schema>, Output>({
    ...
    execute: async (input: unknown) =>
      options.execute(options.inputSchema.parse(input)),
  } as never);
}
...
export function searchToolCatalog(
  ...
  const exactSkillTools = input.skillName
    ? (SKILL_TOOL_MAP[input.skillName] ?? [])
    : [];
...
function selectStepState<TOOLS extends ToolSet>(steps: StepResult<TOOLS>[]) {
  const latestSkill = getLatestToolResult(
    steps,
    "load_skill",
...
function pruneConsumedOrchestrationMessages(
  messages: ModelMessage[],
): ModelMessage[] {
...
export function buildTools(
  ...
  return {
    load_skill: aiTool({
      description:
        "Load a skill by name so the next step can use its instructions.",
...
function formatLogEntry(kind: LogKind, label: string): string {
  const prefix =
    kind === "skill" ? "[skill" : kind === "subagent" ? "[subagent" : "[tool";
